...
#if 1
	printf ("\n");
	printf ("  __________        __    _____   ____ __\n");
	printf (" /_  __/ __ \\      / /   /  _/ | / / //_/\n");
	printf ("  / / / /_/ /_____/ /    / //  |/ / ,<\n");
	printf (" / / / ____/_____/ /____/ // /|  / /| |\n");
	printf ("/_/ /_/         /_____/___/_/ |_/_/ |_|\n");
#endif
	printf ("\n\n%s\n\n", version_string);
	return (0);
}
...
void board_init_r (gd_t *id, ulong dest_addr)
{
	...
	/* board MAC address */
#if defined(MT7620_ASIC_BOARD)		/* Added by Andrew Chang on 2014-08-20 */
	/**********
	 * Physical address blocks for NC400:
	 *    NAME    SIZE     ERASESIZE   NAME       OFFSET
	 *    mtd0  00800000   00010000   ALL        00000000 ~ 007FFFFF (  8MB)
	 *    mtd1  00030000   00010000   Bootloader 00000000 ~ 0002FFFF (192kB)
	 *    mtd2  00010000   00010000   Config     00030000 ~ 0003FFFF ( 64kB)
	 *    mtd3  00010000   00010000   Factory    00040000 ~ 0004FFFF ( 64kB)
	 *    mtd4  00200000   00010000   Kernel     00050000 ~ 0024FFFF (  2MB)
	 *    mtd5  005B0000   00010000   RootFS     00250000 ~ 007FFFFF (  5MB + 704kB)
	 */
	if (1)
	{
		volatile uint8_t *pMacAddr = PHYS_FLASH_START + 0x00040000 + 0x04;
		puts ("ETH address: ");
		for (i = 0; i < 5; ++i)
		{
			bd->bi_enetaddr[i] = pMacAddr[i];
			printf ("%02X:", bd->bi_enetaddr[i]);
		}
		bd->bi_enetaddr[i] = pMacAddr[i] + 1;
		printf ("%02X", bd->bi_enetaddr[i]);
		printf ("%c\n", (char)0x08);
	}
#else
...
#ifdef	AMC_TEST
#define	_CFG_AMC_MD5
//#define	_CFG_AMC_DEBUG


/********/
#ifdef	_CFG_AMC_MD5
#include "md5.h"
int doAmcMd5 ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	char buffer[1024];
	int len;
	unsigned char digest[MV_MD5_MAC_LEN];
	int tmp;
	...
	{
		len = strnlen(argv[1], sizeof(buffer));
		if (len > (sizeof(buffer) - 1))
		{
			printf ("The parameter you entered is too long.\n");
			return -1;
		}
...
U_BOOT_CMD(
	...
	"\nmd5.s string_to_make_md5_test\n"
	"    - calculate the MD5 digest of a string.\n"
	"\nmd5.b mem_address mem_length\n"
	"    - calculate the MD5 digest of a memory.(Not implemented)\n"
);
#endif

/********/
#ifdef	_CFG_AMC_DEBUG
#include <configs/rt2880.h>		/* for MARCO "CFG_LOAD_ADDR" */


#ifdef	CFG_LOAD_ADDR
#define _LOAD_ADDR		CFG_LOAD_ADDR
#else
#define	_LOAD_ADDR		0x80100000
#endif


#ifdef	CFG_CONFIG_SIZE
#define	_CONFIG_SIZE	CFG_CONFIG_SIZE
#else
#define	_CONFIG_SIZE	0x10000
#endif


#ifdef	CFG_ENV_ADDR
#define	_CONFIG_ADDR	(CFG_ENV_ADDR - CFG_FLASH_BASE)
#else
#define	_CONFIG_ADDR	0x30000
#endif


#define	_PARA_OFFSET	0x1400

int doAmcDebug ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	char command[64];
	unsigned long arg;

	/* cp memory to RAM */
	memcpy((void*)_LOAD_ADDR, (void*)(_CONFIG_ADDR + CFG_FLASH_BASE), (size_t)_CONFIG_SIZE);

	if (argc > 1)
	{
		arg = simple_strtoul(argv[1], NULL, 16);
		printf ("Set as 0x%08lx.\n", arg);
		*((volatile unsigned long*)(_LOAD_ADDR + _PARA_OFFSET)) = arg;
	}
	else
	{
		*((volatile unsigned long*)(_LOAD_ADDR + _PARA_OFFSET)) = 0;
	}

	/* erase */
	sprintf (command, "erase %lx %lx", _CONFIG_ADDR, _CONFIG_SIZE);
	run_command(command, 0);

	/* write back */
	sprintf (command, "spi write %lx %lx %lx",	_CONFIG_ADDR, _LOAD_ADDR, _CONFIG_SIZE);		// spi write <addr_to> <addr_from> <len>
	run_command(command, 0);

	/* verify */
	sprintf (command, "md %lx", _CONFIG_ADDR + _PARA_OFFSET + CFG_FLASH_BASE);
	run_command(command, 0);
...
U_BOOT_CMD(
	debug,    2,    1,    doAmcDebug,
	"debug   - Debug command written by Andrew Chang\n",
	"\ndebug\n"
