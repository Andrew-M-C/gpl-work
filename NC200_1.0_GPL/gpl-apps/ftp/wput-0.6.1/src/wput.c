...
static void sigKillHandler()
{
	...
	exit(FTP_ERROR_CODE_NETWORK);
}

static void process_sigpipe()
{
	exit(FTP_ERROR_CODE_UNKNOW);
}
#endif
...
#ifdef MY_FEATURE
	if (0 == opt.failed && 0 != opt.transfered)
	{
		opt.error_code = FTP_ERROR_CODE_SUCCESS;
	}

	return opt.error_code;
#else
