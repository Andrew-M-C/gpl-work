...
int __init rather_probe(struct net_device *dev)
{
	...
	printk("ra_mtd_read_num from 'Factory' GMAC0_OFFSET:%02x GMAC1_OFFSEt:%02x",
		GMAC0_OFFSET, GMAC1_OFFSET);
	
	int idx = 0;
	for( ; idx < sizeof(addr.sa_data); idx++)
		printk("%02x:", addr.sa_data[idx]);
	printk("\n");
...
void rt_gsw_init(void)
{
	...
	*(unsigned long *)(RALINK_ETH_SW_BASE+0x3600) = 0x5e30b;//CPU Port6 Force Link 1G, FC ON
