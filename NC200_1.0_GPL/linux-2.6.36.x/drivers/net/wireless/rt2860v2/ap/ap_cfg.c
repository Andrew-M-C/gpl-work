	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
	...
	while ((this_char = strsep((char **)&pIoctlCmdStr->u.data.pointer, ",")) != NULL) 
	{
		...
		/* replace 0x07 to ',' ,added by zb */
		this_char_copy = this_char;
		while (*this_char_copy)
		{
			if ((CHAR)0x07 == *this_char_copy)
			{
				*this_char_copy = ',';
			}
			this_char_copy++;
...
VOID RTMPIoctlGetConnStatus(
	IN PRTMP_ADAPTER pAd, 
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
#ifdef TP_WSC_LED_SUPPORT
	extern int TpWscRunFlag;
#endif
	...
	if (1 == TpWscRunFlag)
	{
		bConnect = '2'; /* wps is running */
	}
#endif
	wrq->u.data.length = sizeof(BOOLEAN);
    copy_to_user(wrq->u.data.pointer, &bConnect, wrq->u.data.length);
...
/*	msg = (PSTRING)kmalloc(sizeof(CHAR)*(2048), MEM_ALLOC_FLAG); */
    ...
    for (idx = 0; idx < pAd->ApCfg.ApCliTab[0].WscControl.WscProfile.ProfileCnt ; idx++)
    {
		...
		sprintf(msg+strlen(msg), "SSIDLen                         = %d\n", pCredential->SSID.SsidLength);
		...
		sprintf(msg+strlen(msg), "KeyLen                          = %d\n", pCredential->KeyLength);
...
#ifdef TPLINK_MULTI_DEAUTH

	RTMPCancelTimer(&pAd->Mlme.DeauthTimer, &Cancelled);
	if(Enable == 0)
	{
		RTMPSetTimer(&pAd->Mlme.DeauthTimer, 1000);
	}
#endif
	...
	IN	PSTRING			arg)
{
	...
	else if ((strcmp(arg, "WPA2PSK") == 0) || (strcmp(arg, "wpa2psk") == 0) || (strcmp(arg, "WPAPSKWPA2PSK") == 0) || (strcmp(arg, "WPA/WPA2-PSK") == 0) || (strcmp(arg, "WPA2-PSK") == 0))
	{
		pAd->ApCfg.ApCliTab[ifIndex].AuthMode = Ndis802_11AuthModeWPA2PSK;
		if ((strcmp(arg, "WPAPSKWPA2PSK") == 0) || (strcmp(arg, "WPA/WPA2-PSK") == 0))
		{
			pAd->ApCfg.ApCliTab[ifIndex].bMixPskMode |= APCLI_AUTO_WPAAUTH_NONE_ENCRYP;
		}
	}
#ifdef APCLI_WPA_SUPPLICANT_SUPPORT    
...
#if 0
		if ( strcmp(arg, "TKIPAES") == 0 )
		{
			pAd->ApCfg.ApCliTab[ifIndex].bMixPskMode |= APCLI_NONE_WPAAUTH_AUTO_ENCRYP;
		}
#endif
	}    
	else
	...
	IN PSTRING arg)
{
	...
	NdisMoveMemory(ssid, pApCfg->ApCliTab[ifIndex].CfgSsid, pApCfg->ApCliTab[ifIndex].CfgSsidLen);
	/*
...
{
#ifdef TP_WSC_LED_SUPPORT
	extern VOID TPWscSetLEDWscFail();
#endif
	...
	IN	PSTRING			arg)
{
	...
	else if ((strcmp(arg, "WPAPSKWPA2PSK") == 0) || (strcmp(arg, "wpapskwpa2psk") == 0) || (strcmp(arg, "WPA/WPA2-PSK") == 0))
	...
	IN	ULONG					Data)
{
	...
	switch(cmd)
	{
		...
		case CMD_RTPRIV_IOCTL_STATISTICS:
			...
			break;
		case CMD_RTPRIV_IOCTL_CONN_STATUS:
			RTMPIoctlGetConnStatus(pAd, wrq);
			break;
#ifdef WSC_AP_SUPPORT
