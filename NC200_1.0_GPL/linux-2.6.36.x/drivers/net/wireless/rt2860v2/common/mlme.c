	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	do 
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			RTMPInitTimer(pAd, &pAd->Mlme.DeauthTimer, GET_TIMER_FUNCTION(DeauthPeriodicExec), pAd, TRUE);
#endif
		}
#endif /* CONFIG_AP_SUPPORT */
	...
	if (pAd->Mlme.PeriodicRound %MLME_TASK_EXEC_MULTIPLE == 0)
	{
		...
		if ((pAd->CommonCfg.MO_Cfg.bDyncVGAEnable) && (pAd->bCalibrationDone))
		{
			if (pAd->Mlme.PeriodicRound %MLME_TASK_EXEC_MULTIPLE == 0)
			{
				...
				{
					if (BbpReg < (pAd->CommonCfg.MO_Cfg.Stored_BBP_R66 + 0x20))// Upperdebug
					{
						BbpReg += 4;
	...
	IN PVOID SystemSpecific3) 
{
	...
	if(pAd->ApCfg.ApCliTab[0].Enable == 0 && pAd->Mlme.DeuthOutBufferLen)
	{
		MiniportMMRequest(pAd, 0, pAd->Mlme.pDeauthOutBuffer, pAd->Mlme.DeuthOutBufferLen);
	}
