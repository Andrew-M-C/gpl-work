	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	*pCurrState= APCLI_AUTH_REQ_IDLE;
	...
	if(pAd->ApCfg.ApCliTab[ifIndex].Enable == 0)
	{
		BOOLEAN Cancelled;
		RTMPCancelTimer(&pAd->Mlme.DeauthTimer,&Cancelled);
		if(pAd->Mlme.pDeauthOutBuffer)
			MlmeFreeMemory(pAd, pAd->Mlme.pDeauthOutBuffer);
		pAd->Mlme.pDeauthOutBuffer=pOutBuffer;
		pAd->Mlme.DeuthOutBufferLen = FrameLen;
