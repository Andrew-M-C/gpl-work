	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	pApCliEntry = &pAd->ApCfg.ApCliTab[ifIndex];
	mixPskMode = pApCliEntry->bMixPskMode;
	...
	*pCurrState = SYNC_IDLE;
	...
	{
		if (mixPskMode&APCLI_AUTO_WPAAUTH_NONE_ENCRYP && mixPskMode&APCLI_NONE_WPAAUTH_AUTO_ENCRYP)
		{
			POS_COOKIE 	pObj = (POS_COOKIE) pAd->OS_Cookie;
			INT			old_if_type = pObj->ioctl_if_type;
			pObj->ioctl_if_type = INT_APCLI;
						
			if ((pApCliEntry->AuthMode == Ndis802_11AuthModeWPA2PSK) && (pApCliEntry->WepStatus == Ndis802_11Encryption3Enabled))
			{
				DBGPRINT(RT_DEBUG_TRACE, ("APCLI_SYNC - Change to WPA2PSK TKIP\n"));
				Set_ApCli_EncrypType_Proc(pAd, "tkip");
				pAd->ApCfg.ApCliTab[ifIndex].bMixPskMode = mixPskMode;
			}
			else if ((pApCliEntry->AuthMode == Ndis802_11AuthModeWPA2PSK) && (pApCliEntry->WepStatus == Ndis802_11Encryption2Enabled))
			{
				DBGPRINT(RT_DEBUG_TRACE, ("APCLI_SYNC - Change to WPAPSK AES\n"));
				Set_ApCli_AuthMode_Proc(pAd, "wpapsk");
				Set_ApCli_EncrypType_Proc(pAd, "aes");
				pAd->ApCfg.ApCliTab[ifIndex].bMixPskMode = mixPskMode;
			}
			else if ((pApCliEntry->AuthMode == Ndis802_11AuthModeWPAPSK) && (pApCliEntry->WepStatus == Ndis802_11Encryption3Enabled))
			{
				DBGPRINT(RT_DEBUG_TRACE, ("APCLI_SYNC - Change to WPAPSK TKIP\n"));
				Set_ApCli_EncrypType_Proc(pAd, "tkip");
				pAd->ApCfg.ApCliTab[ifIndex].bMixPskMode = mixPskMode;
			}
			else if ((pApCliEntry->AuthMode == Ndis802_11AuthModeWPAPSK) && (pApCliEntry->WepStatus == Ndis802_11Encryption2Enabled))
			{
				DBGPRINT(RT_DEBUG_TRACE, ("APCLI_SYNC - Change to WPA2PSK AES\n"));
			...
			{
				pAd->ApCfg.ApCliTab[ifIndex].bMixPskMode = APCLI_NONE_WPAAUTH_NONE_ENCRYP;
				DBGPRINT(RT_DEBUG_ERROR, ("%s: Not support security mode [%d:%d]!!!\n", __FUNCTION__, pApCliEntry->AuthMode, pApCliEntry->WepStatus));
			}
			pObj->ioctl_if_type = old_if_type;
		}
		else if (mixPskMode&APCLI_NONE_WPAAUTH_AUTO_ENCRYP)
		{
			POS_COOKIE 	pObj = (POS_COOKIE) pAd->OS_Cookie;
			INT			old_if_type = pObj->ioctl_if_type;
			pObj->ioctl_if_type = INT_APCLI;
			...
			{
				DBGPRINT(RT_DEBUG_TRACE, ("APCLI_SYNC - Change to tkip\n"));
				Set_ApCli_EncrypType_Proc(pAd, "tkip");
				pAd->ApCfg.ApCliTab[ifIndex].bMixPskMode = mixPskMode;			
			}
			else if (pApCliEntry->WepStatus == Ndis802_11Encryption2Enabled)
			{
				DBGPRINT(RT_DEBUG_TRACE, ("APCLI_SYNC - Change to aes\n"));
				Set_ApCli_EncrypType_Proc(pAd, "aes");
				pAd->ApCfg.ApCliTab[ifIndex].bMixPskMode = mixPskMode;						
			}
			else 
		...
		else if (mixPskMode&APCLI_AUTO_WPAAUTH_NONE_ENCRYP)
		{
			POS_COOKIE 	pObj = (POS_COOKIE) pAd->OS_Cookie;
			INT			old_if_type = pObj->ioctl_if_type;
			pObj->ioctl_if_type = INT_APCLI;
			...
			{
				DBGPRINT(RT_DEBUG_TRACE, ("APCLI_SYNC - Change to WPAPSK\n"));
				Set_ApCli_AuthMode_Proc(pAd, "wpapsk");
				pAd->ApCfg.ApCliTab[ifIndex].bMixPskMode = mixPskMode;			
			}
			else if (pApCliEntry->AuthMode == Ndis802_11AuthModeWPAPSK)
			{
				DBGPRINT(RT_DEBUG_TRACE, ("APCLI_SYNC - Change to WPA2PSK\n"));
				Set_ApCli_AuthMode_Proc(pAd, "wpa2psk");
				pAd->ApCfg.ApCliTab[ifIndex].bMixPskMode = mixPskMode;			
			}
			else 
