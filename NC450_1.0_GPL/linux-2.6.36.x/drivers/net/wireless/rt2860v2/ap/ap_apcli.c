	...
	*/
	for (entryIdx=0; entryIdx<pScanTab->BssNr;entryIdx++)
	{
		...
		if (NdisEqualMemory(pCfgSsid, pBssEntry->Ssid, CfgSsidLen) &&
						(pBssEntry->SsidLen) &&		
		//if ((NdisEqualMemory(pCfgSsid, pBssEntry->Ssid, CfgSsidLen) || 0 == pBssEntry->SsidLen) &&
	...
	*/
	...
	if ((pSsidBssTab->BssNr == 0))
	{
		...
		pAd->ApCfg.ApCliAutoConnectRunning = FALSE;
			...
			pSsidBssTab->BssNr <=MAX_LEN_OF_BSS_TABLE)
	{	
		/* added by zb for HtExtcha auto switch *2013/11/28 */
		if (pAd->CommonCfg.HtCapability.HtCapInfo.ChannelWidth  == BW_40)
		{
			if (pSsidBssTab->BssEntry[pSsidBssTab->BssNr -1].AddHtInfo.AddHtInfo.ExtChanOffset == EXTCHA_ABOVE)
					Set_HtExtcha_Proc(pAd, "1");
			else if (pSsidBssTab->BssEntry[pSsidBssTab->BssNr -1].AddHtInfo.AddHtInfo.ExtChanOffset == EXTCHA_BELOW)
