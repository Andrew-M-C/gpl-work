	...
	for (;;) {
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
				...
				bb_info_msg("No lease, fallback ip set, the parent id : %d.\n", getppid());
			...
			case RENEWING:
				//if (timeout > 60) {
				if (timeout > 0) { /* 60 secs to 0 sec, modified by zb, 2014/1/8 */
		 ...
		 * But it wasn't a packet. It's a signal pipe then. */
		{
			...
			case SIGUSR2:
				...
				timeout = INT_MAX;
				break;
			case SIGWINCH: /* signal reused for host name is updated, by zb, 2013/12/21 */
				if((dhcpcFp = fopen("/usr/local/config/ipcamera/system.conf", "r")) != NULL)
				{
					while (fgets(dhcpcLineGet, sizeof(dhcpcLineGet), dhcpcFp) != NULL)
					{
						if(NULL != strstr(dhcpcLineGet,"SYSTEM_PRODUCT_NAME"))
						{
							client_config.hostname[OPT_LEN] = strlen(dhcpcLineGet) - strlen("SYSTEM_PRODUCT_NAME=\n");
							memcpy(client_config.hostname + OPT_DATA, dhcpcLineGet +  strlen("SYSTEM_PRODUCT_NAME="), strlen(dhcpcLineGet) - strlen("SYSTEM_PRODUCT_NAME=\n"));
							break;
						}
					}
				}
