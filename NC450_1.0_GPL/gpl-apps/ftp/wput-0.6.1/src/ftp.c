...
/* if execute this function on mips, it would take some error */
#ifndef MY_FEATURE
	printout(vLESS,
			"--%s-- `%s'\n"
  ...
  while(!fsession->done && ( fsession->retry > 0 || fsession->retry == -1) ){
	...
	if(!fsession->ftp->sock) {
		if(ftp_connect(fsession->ftp, &opt.ps) == ERR_FAILED) {
			...
			if (-1 == opt.error_code)
			{
				opt.error_code = FTP_ERROR_CODE_NETWORK;
			}
#endif
			continue;
		}
	...
	if( res < 0 ) {
		...
		if (-1 == opt.error_code)
		{
			opt.error_code = FTP_ERROR_CODE_LOGIN;
		}
#endif
		return ERR_FAILED;
	}
		...
		strcmp(fsession->ftp->current_directory, fsession->target_dname))))
	{
		...
		if( res == ERR_FAILED ) {
			...
			if(res == ERR_FAILED) {
				...
				if (-1 == opt.error_code)
				{
					if (fsession && fsession->ftp 
						&& (NULL != strstr(fsession->ftp->r.message, "Permission denied") 
							|| NULL != strstr(fsession->ftp->r.message,"Forbidden")))
					{
						opt.error_code = FTP_ERROR_CODE_PERMISSION;
					}
					else
					{
						opt.error_code = FTP_ERROR_CODE_PATH;
					}
				}
#endif
				return ERR_FAILED;
