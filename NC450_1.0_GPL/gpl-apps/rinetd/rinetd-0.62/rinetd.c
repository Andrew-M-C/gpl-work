...
void readConfiguration(void)
{
	...
	int di;
	/*init run state*/
	system("echo '0'>/tmp/rinetd_run_state"); /*init to 0 , fail*/
	...
	while (1) {
		...
		if (!strcmp(bindAddress, "allow")) {
			...
			system("echo '1'>/tmp/rinetd_run_state"); /*run to here , is succeed*/
...
void handleAccept(int i)
{
	...
	if (index == -1) {
		...
		// change by whh, disallow auto enlargement
		goto shortage;
