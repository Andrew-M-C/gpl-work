...
const uchar		NetBcastAddr[6] =	/* Ethernet bcast address		*/
...
NetSendUDPPacket(uchar *ether, IPaddr_t dest, int dport, int sport, int len)
{
	...
	if (memcmp(ether, NetEtherNullAddr, 6) == 0) {
		...
		NetArpWaitSPort = sport;
		NetArpWaitDPort = dport;
	...
	case PROT_ARP:
		...
		case ARPOP_REPLY:		/* arp reply */
			...
			if (tmp == NetArpWaitReplyIP) {
				...
				else if (NetArpWaitTxPacketSize)		/* Added by Andrew Chang on 2014-08-18 */
				{
					puts("Now send cached UDP packet.\n");
...
#define	DB_UDP_XSUM(x)			/* define as nothing */
unsigned
NetUdpCksum(IP_t *ip, int len)
{
	ulong	xsum = 0;
	uchar	*data;
	len += 8;
	...
	/* sum IP protocol */
	xsum += (ushort)(ip->ip_p);
	DB_UDP_XSUM(printf("sum: 0x%04X protocol\n", xsum));

	/* sum UDP length */
	data = &(ip->udp_len);
	xsum += ((ulong)(*(data + 0))) << 8;
	xsum += ((ulong)(*(data + 1))) << 0;
	DB_UDP_XSUM(printf("sum: 0x%04X UDP len\n", xsum));

	/* sum UDP content */
		/* odd length check */
	DB_UDP_XSUM(printf("sum: %d UDP length\n", len));
#if 0
	if (len & 1)
	{
		printf("UDPSUM: len odd\n");
		xsum += *(((uchar*)ip) + IP_HDR_SIZE + len); 
		len --;
	}
	...
	while(len > 0)
	{
		xsum += ((ulong)(*(data + 0))) << 8;
		xsum += ((ulong)(*(data + 1))) << 0;
		DB_UDP_XSUM(printf("\t0x%02X%02X (0x%04X)\n", ((ulong)(*(data + 0))), ((ulong)(*(data + 1))), xsum));
		data += 2;
		len -= 2;
	}
#else	/* Modified by Andrew on 2014-08-18 */
	data = &(ip->udp_src);
	while(len > 1)
	{
		xsum += ((ulong)(*(data + 0))) << 8;
	...
	if (len)
	{
		DB_UDP_XSUM(printf("UDPSUM: len odd\n"));
		xsum += ((ulong)(*(data))) << 8; 
		len --;
	}
#endif

	DB_UDP_XSUM(printf("sum: 0x%04X ~", xsum));

	xsum = (xsum & 0x0000FFFF) + (xsum >> 16);
	xsum = (xsum & 0x0000FFFF) + (xsum >> 16);

	DB_UDP_XSUM(printf("0x%04X final\n", (ushort)(~(xsum & 0x0000FFFF))));
...
NetSetIP(volatile uchar * xip, IPaddr_t dest, int dport, int sport, int len)
{
	...
	/* UDP checksum added by andrew Chang on 2014-08-12 */
