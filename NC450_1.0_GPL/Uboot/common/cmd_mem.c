...
#define RT2880_U_BOOT_CMD_OPEN 1
...
int do_mem_md ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	do {
		...
		for (i=0; i<linebytes; i+= size) {
			if (size == 4) {
				printf(" %08x", (*uip++ = *((volatile uint *)addr)));
			} else if (size == 2) {
				printf(" %04x", (*usp++ = *((volatile ushort *)addr)));
			} else {
				printf(" %02x", (*ucp++ = *((volatile u_char *)addr)));
			}
			addr += size;
		}
#ifdef CONFIG_HAS_DATAFLASH
...
mod_mem(cmd_tbl_t *cmdtp, int incrflag, int flag, int argc, char *argv[])
{
	...
	do {
		printf("%08lx:", addr);
		if (size == 4)
			printf(" %08x", *((volatile uint   *)addr));
		else if (size == 2)
			printf(" %04x", *((volatile ushort *)addr));
		else
			printf(" %02x", *((volatile u_char *)addr));
...
#endif
				if (size == 4)
					*((volatile uint   *)addr) = i;
				else if (size == 2)
					*((volatile ushort *)addr) = i;
				else
					*((volatile u_char *)addr) = i;
				if (incrflag)
					addr += size;
			}
		}
