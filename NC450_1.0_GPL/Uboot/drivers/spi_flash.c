...
int do_flerase (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	else if (strcmp(argv[1], "uboot") == 0) 
	{
		...
		unsigned int addr_from, len;

		addr_from = simple_strtoul(argv[1], NULL, 16);		
		len = simple_strtoul(argv[2], NULL, 16);
		if (addr_from + len > spi_chip_info->sector_size * spi_chip_info->n_sectors) {
			printf("\n Invalid flash memory range !!\n");
			return 1;
		}
			
		printf("\n Erase flash sectors !!\n");
		raspi_unprotect();
		rcode = raspi_erase(addr_from, len);
		return rcode;
	}
...
int ralink_spi_command(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	if (!strncmp(argv[1], "id", 3)) {
		...
		for (i = 0; i < len; i++) {
			...
			printf("%s%02x ", i%8 ? "" : " ", p[i]);
		}
		printf("\n");
		free(p);
	}
	else if (!strncmp(argv[1], "write", 6)) {
		unsigned int addr_from, addr_to, len, abs;
		int ret;
		...
		addr_from = simple_strtoul(argv[3], NULL, 16);
		abs = addr_from > addr_to ? addr_from - addr_to : addr_to - addr_from;		
		len = simple_strtoul(argv[4], NULL, 16);
		if ( !abs || abs < len ) {
			printf("parameter error\n");
		} else if ( (ret=raspi_write((char*)addr_from, addr_to, len)) != len) {
			printf("error %d\n", ret);
		}
	}
...
U_BOOT_CMD(
	...
	"  spi sr write <value>\n"
	"  spi read <addr> <len>\n"
	"  spi write <addr_to> <addr_from> <len>\n"
);
#endif
